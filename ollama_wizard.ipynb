...
class EnhancedOllamaSetupColab:
    def __init__(self):
        self.cloudflare_token = "eyJhIjoiYzRhMTcwZmEzZGViOWRkNTRmNmQ1NTdkMmVlNjg2MTAiLCJ0IjoiMTBjN2Q3YzgtZWQwOC00ZTg1LThiMjYtYjBmMDg0ODliNTA1IiwicyI6Ik1qRTBORGt3Wm1JdE4yVTVNeTAwTmpGaUxXRTBNamd0WVdFeE9HVmlZamN4TVRNNSJ9"
        self.ollama_models = ["hf.co/TheDrummer/Gemmasutra-Mini-2B-v1-GGUF:Q6_K", "hf.co/TheBloke/Luna-AI-Llama2-Uncensored-GGUF:Q6_K"]  # Default models list
    ...
    def install_ollama_model(self, model_name, model_index=1, total_models=1):
        """Install single Ollama model with real-time progress for Colab"""
        self.print_step(5, 5, f"Installing Model {model_index}/{total_models}: {model_name}", "🧠")
        ...
        command = f"ollama pull '{model_name}'"  # Added quotes to handle model names with special chars
    ...
    def install_all_models(self):
        """Install all models in the list"""
        if not self.ollama_models:
            self.display_warning("⚠️ No models to install!")
            return True
            
        total_models = len(self.ollama_models)
        failed_models = []
        
        for i, model_name in enumerate(self.ollama_models, 1):
            success = self.install_ollama_model(model_name, i, total_models)
            if not success:
                failed_models.append(model_name)
                
        if failed_models:
            self.display_error(f"💥 Failed to install models: {', '.join(failed_models)}")
            return False
        else:
            self.display_success(f"🎉 All {total_models} models installed successfully!")
            return True

    def show_completion_summary(self, ollama_pid, tunnel_pid):
        """Show beautiful completion summary with HTML"""
        models_list = ", ".join(self.ollama_models)
        completion_html = f"""
        <div style="
        ...
        ">
            ...
            ">
                ...
                <p style="margin: 5px 0;">✅ Models installed ({len(self.ollama_models)}): {models_list}</p>
    ...
    def setup_all(self):
        ...
        try:
            ...
            if not self.install_all_models():
                return False
...
def run_ollama_setup(custom_models=None):
    """
    Main function to run in Google Colab
    Usage: 
        result = run_ollama_setup()
        # or with custom models:
        result = run_ollama_setup(["llama2:7b", "deepseek-r1:1.5b"])
        # or with single model:
        result = run_ollama_setup("llama2:7b")
    """
    setup = EnhancedOllamaSetupColab()
    
    if custom_models:
        # Handle both single model (string) and multiple models (list)
        if isinstance(custom_models, str):
            setup.ollama_models = [custom_models]
        elif isinstance(custom_models, list):
            setup.ollama_models = custom_models
        else:
            setup.ollama_models = [str(custom_models)]
        
        # Check if running in Colab to display HTML
        if 'google.colab' in sys.modules:
            models_display = ", ".join(setup.ollama_models)
            display(HTML(f"""
            <div style="background: #e3f2fd; padding: 10px; border-radius: 5px; margin: 10px 0;">
                <strong>🎯 Using custom models ({len(setup.ollama_models)}): {models_display}</strong>
            </div>
            """))
        else:
            models_display = ", ".join(setup.ollama_models)
            print(f"{Colors.CYAN}🎯 Using custom models ({len(setup.ollama_models)}): {models_display}{Colors.RESET}")
...
def main():
    ...
    # Handle command line arguments properly - support multiple models
    if len(sys.argv) > 1:
        models = []
        for arg in sys.argv[1:]:
            # Skip common flags and invalid model names
        ...
        if models:
            ...
            if 'google.colab' in sys.modules:
                ...
                display(HTML(f"""
                <div style="background: #e3f2fd; padding: 10px; border-radius: 5px; margin: 10px 0;">
                    <strong>🎯 Using custom models ({len(models)}): {models_display}</strong>
                </div>
                """))
            else:
                models_display = ", ".join(models)
                print(f"{Colors.CYAN}🎯 Using custom models ({len(models)}): {models_display}{Colors.RESET}")
